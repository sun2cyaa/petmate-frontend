...
export default function AddressFormModal({
  ...
  }, [address, show, isEditMode]); // ← searchQuery 제거

  // 검색어가 바뀌면 address만 동기화 (type은 건드리지 않음)
  useEffect(() => {
    if (searchQuery) {
  ...
  useEffect(() => {
    ...
    if (!window.kakao) {
      ...
      kakaoScript.src = `https://dapi.kakao.com/v2/maps/sdk.js?appkey=${process.env.REACT_APP_KAKAO_MAP_KEY}&autoload=false&libraries=services`;
      kakaoScript.onload = () => {
      ...
      oncomplete: function (data) {
        ...
        const searchCoordinates = () => {
          if (window.kakao && window.kakao.maps && window.kakao.maps.services) {
            ...
            console.warn(
              "Kakao 지도 API가 아직 로드되지 않았습니다. 잠시 후 다시 시도합니다."
            );
  ...
  const handleSave = () => {
    ...
    const { coordinates, postcode } = mapSelectedLocation
      ? {
          // 지도에서 선택(최우선)
          coordinates: {
      ...
      ? {
          // 다음 우편번호로만 선택
          coordinates: null, // 좌표는 없음(백엔드에서 필요 시 지오코딩)
      ...
      <div className="address_modal_content">
        ...
        <div className="address_modal_form">
              ...
              {addressTypes.map((type) => {
                ...
                return (
                  <button
                    ...
                    className={`address_type_button ${
                      formData.type === type.type ? "selected" : ""
                    }`}
                  >
          ...
          <div className="address_checkbox_group">
            ...
            />
            <label
              htmlFor={isEditMode ? "editDefault" : "default"}
              className="address_checkbox_label"
            >
              기본 주소로 설정
            </label>
          </div>
