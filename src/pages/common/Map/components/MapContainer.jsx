...
function MapContainer({
  ...
  // 원형 마커 이미지 생성
  const createCircleMarker = useCallback(
    (iconSrc, color = "#eb9666") =>
      new Promise((resolve) => {
        const SCALE = 2;
        const W = 36,
          H = 36;
  ...
  // 물방울 마커 이미지 생성
  const createDropMarker = useCallback(
    (iconSrc, color = "#eb9666") =>
      new Promise((resolve) => {
        const SCALE = 2;
        const W = 48,
  ...
  // 서비스별 마커 이미지
  const getMarkerImageForService = useCallback(
    async (serviceCode, isSelected = false) => {
      const markerConfig = {
      ...
      try {
        ...
        const userMarkerImage = new window.kakao.maps.MarkerImage(
          ...
          new window.kakao.maps.Size(
            userMarkerData.size.w,
            userMarkerData.size.h
          ),
          {
            offset: new window.kakao.maps.Point(
              userMarkerData.offset.x,
              userMarkerData.offset.y
            ),
          }
        );
        ...
        window.kakao.maps.event.addListener(userMarker, "dragend", function () {
          ...
          console.log(
            `사용자가 마커를 드래그로 이동: 위도 ${newLat}, 경도 ${newLng}`
          );
        });
        ...
        window.kakao.maps.event.addListener(
          ...
          function () {
            ...
            onLocationChange(newLat, newLng);
            console.log(
              `기본 마커 드래그로 이동: 위도 ${newLat}, 경도 ${newLng}`
            );
          }
        );
  ...
  const updateUserMarkerToCenter = useCallback(() => {
    ...
    console.log(
      `지도 중심 변경으로 마커 이동: 위도 ${centerLat}, 경도 ${centerLng}`
    );
  }, []);
      ...
      (error) => {
        ...
        switch (error.code) {
          case error.PERMISSION_DENIED:
            errorMessage =
              "위치 접근이 거부되었습니다. 브라우저 설정을 확인해주세요.";
            break;
  ...
  useEffect(() => {
    ...
    createUserMarker(map, userPosition)
      .then((marker) => {
        if (marker) {
      ...
      try {
        ...
        for (const company of companies) {
          ...
          try {
            ...
            // 기본/선택 이미지 모두 준비
            const normalData = await getMarkerImageForService(
              company.repService,
              false
            );
            const selectedData = await getMarkerImageForService(
              company.repService,
              true
            );
            ...
            const selectedImage = new window.kakao.maps.MarkerImage(
              ...
              {
                offset: new window.kakao.maps.Point(
                  selectedData.offset.x,
                  selectedData.offset.y
                ),
              }
            );
